...
class  adminback
{
    ...
    function xetduyet()
    {
        $sql = "SELECT COUNT(*) AS count FROM sanpham WHERE trangthai = 'dangchoxetduyet'";
    ...
    function nongdan_login($data)
    {
        $nongdan_email = $data["nongdan_email"];
        $nongdan_pass = md5($data['nongdan_pass']);
        $role_default = 'Nongdan';
        $query = "SELECT * FROM `taikhoan` WHERE email = '$nongdan_email' AND matkhau = '$nongdan_pass' AND role = '$role_default'";

        if (mysqli_query($this->connection, $query)) {
            $result = mysqli_query($this->connection, $query);
            $nongdan_info = mysqli_fetch_assoc($result);
            if ($nongdan_info) {
                header("location:dashborad.php");
                session_start();
                $_SESSION['id_acc'] = $nongdan_info['id_acc'];
                $_SESSION['nongdan_email'] = $nongdan_info['email'];
                $_SESSION['role'] = $nongdan_info['role'];
            } else {
                $log_msg = "Sai email hoặc mật khẩu!!";
                return $log_msg;
            }
        }
    }
    ...
    function add_catagory($data)
    {
        ...
        if (mysqli_query($this->connection, $query)) {
            return "Thêm danh mục {$ctg_name} thành công!!";
        } else {
    ...
    function updata_catagory($data)
    {
        ...
        if (mysqli_query($this->connection, $query)) {
            return "Cập nhật danh mục {$u_ctg_name} thành công!!";
        }
    }

    function add_vsx($data)
    {
        $vsx_nguoidang = $data['vsx_nguoidang'];
        $vsx_ten = $data['vsx_ten'];
        $vsx_mavung = $data['vsx_mavung'];
        $vsx_hinhanh = $data['vsx_hinhanh'];
        $vsx_sdt = $data['vsx_sdt'];
        $vsx_diachi = $data['vsx_diachi'];
        $vsx_dientich = $data['vsx_dientich'];

        $query = "INSERT INTO `vungsanxuat`(`nguoidang`,`tenvung`,`mavung`,`hinhanh`,`sdt`,`diachi`,`dientich`) VALUES 
        ($vsx_nguoidang,'$vsx_ten','$vsx_mavung','$vsx_hinhanh','$vsx_sdt','$vsx_diachi','$vsx_dientich')";

        if (mysqli_query($this->connection, $query)) {
            return "Thêm vùng sản xuất {$vsx_ten} thành công!!";
        } else {
            return "Thêm vùng sản xuất thất bại!!";
        }
    }

    function display_vsx($data)
    {
        $query = "SELECT * FROM `vungsanxuat` WHERE nguoidang = '$data'";
